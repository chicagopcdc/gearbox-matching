...
def get_presigned_url(request, key_name, pu_config, method):
    ...
    presigned_url = ''
    try:
        presigned_url = request.app.boto_manager.presigned_url(bucket_name,key_name, config.S3_PRESIGNED_URL_EXPIRES, pu_config, method, config.DUMMY_S3 )
...
def get_object(request, bucket_name, key_name, expires, boto_params=[], method=None):
    ...
    if config.DUMMY_S3:
        ...
        try:
            ...
            logger.info(f"HTTP Error: {ex} fetching bucket: {bucket_name} key: {key_name}")
            raise HTTPException(status.HTTP_500_INTERNAL_SERVER_ERROR,
        ...
        try:
            expires = 300
            response = request.app.boto_manager.get_object(bucket=bucket_name, key=key_name, expires=expires, config=boto_params) 
