...
from gearboxdatamodel.crud.match_conditions import get_algorithm_engines
...
async def get_match_conditions(session):
    ...
    for ae in all_algo_engs:
        if ae.study_algo_engine.study_version.study_id in ae_dict:
            ae_dict[ae.study_algo_engine.study_version.study_id].append(
                (ae.path, ae.sequence)
            )
        else:
            ae_dict[ae.study_algo_engine.study_version.study_id] = [
                (ae.path, ae.sequence)
            ]
...
def build_tree(nodelist):
    ...
    for i in range(len(nodelist)):
        ...
        if len(crit_que) == 0:
            ...
            ################################################################
            if working_node["criteria"][-1] == node and (
                neighbor_group_id == working_group_id or neighbor_group_id == ""
            ):
            ...
            ################################################################
                ...
                #################################################################################
                    ...
                    while (
                        working_node["operator"] != neighbor_operator
                        and neighbor_operator
                    ):
                        try:
                            working_node = crit_que.pop()
                            if type(working_node) is tuple:
                ...
                if neighbor == check_ahead_node:
                    ...
                    ###################################################################
                        ...
                        ###################################################################
                            ...
                            working_node["criteria"] = working_node["criteria"][
                                : node_idx + 1
                            ]
...
def get_tree(full_paths, study_id=None, suppress_header=False):
    ...
    if suppress_header:
        ...
        return_criteria = {"studyId": study_id, "algorithm": criteria_tree}
