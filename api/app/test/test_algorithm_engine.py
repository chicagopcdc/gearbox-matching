...
def algorithm_engineA():
    return AlgorithmEngine(el_criteria_has_criterion_id = 3, parent_id = 1, parent_path = 'parentPathA')


@pytest.fixture(scope="module")
def algorithm_engineB():
    return AlgorithmEngine(el_criteria_has_criterion_id = 4, parent_id = 2, parent_path = 'parentPathB')
...
def test_algorithm_engine_info(algorithm_engineA, algorithm_engineB, app, session):
    for ae in [algorithm_engineA.as_dict(), algorithm_engineB.as_dict()]:
        with app.test_request_context("/algorithm_engine/{}".format(ae['id']), method="GET"):
            pid = "{}".format(ae['id'])
...
def test_create_algorithm_engine(app, session):
    payload= {'el_criteria_has_criterion_id': 5, 'parent_id': 1, 'parent_path': 'p-path'}
...
def test_update_algorithm_engine(algorithm_engineA, algorithm_engineB, app, session):
    ...
    pidA = "{}".format(algorithm_engineA_dict['id'])
